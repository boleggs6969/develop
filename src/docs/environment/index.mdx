...
*NOTE*: There's some documented known issues at the bottom of this page.
...
NOTE: There's an issue with the latest Docker, thus, we need to uninstall what `brew bundle` installed and then downgrade with:

```shell
brew uninstall docker && brew cask install \
  https://raw.githubusercontent.com/Homebrew/homebrew-cask/3ae8ea3cabebeb1fc0caababdfc739a910d8e0cf/Casks/docker.rb
```
...
**NOTE**: We've recently switched our development environment to use Python 3 by default. Temporarily you will still be able
to set things up with Python 2 by running this command (it will be read by the setup scripts):

```
export SENTRY_PYTHON2=1
```

**NOTE**: If you are switching your development environment from the default Python 2 set up you will need to run `make clean`.
